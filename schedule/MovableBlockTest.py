...
def main():
    ...
    r1: int = canvas.create_rectangle(50, 50, 150, 150, fill='blue',
                                      tags=(MOVABLE_TAG_NAME, "obj1"))
...
def select_gui_block_to_move(canvas: Canvas, event: Event):
    ...
    tags: tuple[str] = tuple(
        filter(lambda x: x != MOVABLE_TAG_NAME and x != 'current', canvas.gettags(obj[0])))
    ...
    canvas.bind("<Motion>", lambda e: gui_block_is_moving(canvas, tags[0], event.x, event.y, e))
...
def gui_block_is_moving(canvas, tag, x, y, event):
    ...
    # rebind for motion
    canvas.bind("<Motion>", lambda e: gui_block_is_moving(canvas, tag, event.x, event.y, e))
